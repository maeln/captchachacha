...
impl ShaderManager {
    ...
    pub fn compile_shader(&mut self, shader_src: &str, shader_type: ShaderType) -> Shader {
        let uniforms = Shader::parse_uniforms(shader_src);
        let id = Shader::compile_shader(shader_src, &shader_type).unwrap();
        let shd = Shader {
            addr: id,
            path: "".to_string(),
            uniforms,
    ...
    pub fn mk_program(&mut self, shaders: &Vec<Arc<Mutex<Shader>>>) -> u32 {
        let prog = Program::load_program(shaders).unwrap();
        let mut db = self.db.lock().unwrap();
        db.add(Arc::new(Mutex::new(prog)))
    }
...
impl Shader {
    ...
    pub fn load_shader(path: &Path) -> Option<Shader> {
        ...
        let addr = Shader::compile_shader(&src, &shader_type);
        if addr.is_none() {
            return None;
        }
    ...
    pub fn reload(&mut self) {
        ...
        let src = src.unwrap();
        let new_addr = Shader::compile_shader(&src, &self.shader_type);
